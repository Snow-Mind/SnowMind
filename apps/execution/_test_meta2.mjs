...
// Now import and test accountMetadata (handle BigInt serialization)
try {
  const { accountMetadata } = await import('@zerodev/sdk');
  const result = await accountMetadata(client, ACCOUNT, '0.3.1');
  console.log('accountMetadata result:', JSON.stringify(result, (key, value) => typeof value === 'bigint' ? value.toString() : value));
} catch(e) {
  console.log('accountMetadata error:', e.message);
  console.log(e.stack);
}
