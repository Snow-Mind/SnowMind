...
class TestAutoRebalanceExecution:
    ...
    @pytest.mark.asyncio
    async def test_scheduler_records_tick_when_no_active_accounts(self, rebalancer):
        """No-op cycles with zero active accounts should still heartbeat watchdog."""
        with patch("app.workers.scheduler.get_settings") as gs, \
             patch("app.workers.scheduler.get_supabase") as gdb, \
             patch("app.workers.scheduler.check_paymaster_balance", new_callable=AsyncMock) as cpm, \
             patch("app.workers.scheduler.scheduler_watchdog") as wd:
    ...
    @pytest.mark.asyncio
    async def test_scheduler_records_tick_when_no_valid_session_keys(self, rebalancer):
        """No-op cycles with only invalid/expired session keys should heartbeat watchdog."""
        account_id = str(uuid4())
        address = "0x4006ce775C928E4e4dE5BAC01d9d69Ed3a793556"
