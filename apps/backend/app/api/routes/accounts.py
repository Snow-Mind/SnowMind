...
# ── Admin: update session key allowed protocols ────────────────

ALL_PROTOCOL_IDS = {"aave_v3", "benqi", "spark", "euler_v2", "silo_savusd_usdc", "silo_susdp_usdc"}


class UpdateAllowedProtocolsRequest(BaseModel):
    allowed_protocols: list[str] = Field(
        ...,
        alias="allowedProtocols",
        description="List of protocol IDs to allow",
    )
    model_config = {"populate_by_name": True}


@router.patch("/{address}/session-key/allowed-protocols")
@limiter.limit("10/minute")
async def update_allowed_protocols(
    request: Request,
    address: str,
    req: UpdateAllowedProtocolsRequest,
    db: Client = Depends(get_db),
    _auth: dict = Depends(require_privy_auth),
):
    """Update allowed_protocols on the active session key without replacing the key itself.

    The on-chain session key already has permissions for all protocols,
    so this only updates the DB-level filter used by the rebalancer.
    """
    address = validate_eth_address(address)
    invalid = set(req.allowed_protocols) - ALL_PROTOCOL_IDS
    if invalid:
        raise HTTPException(status_code=400, detail=f"Unknown protocol IDs: {sorted(invalid)}")
    ...
    # Update the most recent active session key
    keys = (
        db.table("session_keys")
        .select("id")
    ...
    if not keys.data:
        raise HTTPException(status_code=404, detail="No active session key found")

    db.table("session_keys").update(
        {"allowed_protocols": req.allowed_protocols}
    ).eq("id", keys.data[0]["id"]).execute()

    logger.info(
        "Updated allowed_protocols for %s to %s",
        address, req.allowed_protocols,
    )
    return {"allowedProtocols": req.allowed_protocols}
